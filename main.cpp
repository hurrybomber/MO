...
inline void bad_input() {std::cout << "Введено неверное значение!" << std::endl << "Продолжите ввод с последенего верно введенного значения: ";}
...
int main() {
    ...
    std::cout << "Введите коэффициенты F: ";
    T[row-1][0] = 0;
    ...
    std::cout << "Введите матрицу системы ограничений: ";
    ...
    std::cout << "Введите вектор системы ограничений: ";
    for (unsigned int i = 0; i < row - 1; i++){
        std::cin >> T[i][0];
        while (!std::cin) {
            bad_input();
    ...
    while(!Flag_optimal_solution_found) {
        ...
        if(!flag){
            ...
            search_solving_col_for_optimal(table, solving_col);
            if (solving_col == 0) {
                std::cout << "Найденное решение является оптимальным." << std::endl
                << "Fmax= "<< -T[row-1][0] << std::endl;
                Flag_optimal_solution_found = true;
                break;
            }
        }
        else{
            unsigned int row_with_negative_in_si0 = search_for_negative_in_si0(table);
